...
(test-group "s16vector/searching"
  ...
  (test "any" 10 (s16vector-any (lambda (x y) (and (even? x) (even? y) (+ x y)))
                                s5 (s16vector 0 1 2 6 4)))
  (test "any" #f (s16vector-any (lambda (x y) (and (even? x) (even? y) (+ x y)))
                                s5 (s16vector 0 1 2 5 4)))
  (test "every" 10 (s16vector-every (lambda (x) (and (exact? x) (* x 2))) s5))
  (test "every" 10 (s16vector-every (lambda (x y) (and (exact? x) (exact? y) (+ x y)))
                                    s5 s5))
  (test-assert "not every" (not (s16vector-every odd? s5)))
  (test-assert "not every" (not (s16vector-every < s5 (s16vector 10 10 10 10 0))))
