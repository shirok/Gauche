...
(define (match&list rx input)
  (cond [(rxmatch rx input)
         => (^[match] (map (^n (rxmatch-substring match n))
                           (iota (rxmatch-num-matches match))))]
        [else #f]))

(define (test-re re str expect)
  (test* (write-to-string `(,re ,str)) expect (match&list re str)))
...
(test-re #/abc$|def/   "defabc" '("def"))
(test-re #/^abc|def$/  "abcdef" '("abc"))
(test-re #/^abc|def$/  "defabc" #f)
(test-re #/^abc|def$/  "defabc" #f)
(test-re #/(^abc|def$)/ "aaadef" '("def" "def"))
(test-re #/(^abc|def$)$/ "aaadef" #f)
(test-re #/(^abc|def$)$/ "aaadef$" '("def$" "def$"))
(test-re #/(abc$|def)$/ "aaabc" #f)
...
(test-re #/a[bc]d/   "aed" #f)
(test-re #/a[a-z]d/  "aed" '("aed"))
(test-re #/a[a-z]d/  "aEd" #f)
(test-re #/a[]]d/    "a]d" '("a]d"))
(test-re #/a[]-]d/   "a-d" '("a-d"))
(test-re #/a[]-^]d/  "a-d" #f)
(test-re #/a[]-^]d/  "a]d" '("a]d"))
...
(test-re #/(?>.*\/)foo/ "/this/is/a/long/line/" #f)
...
(test-re #/^(\D*)(?=\d)(?!123)/ "abc123" #f)
(test-re #/(?!^)abc/ "the abc" '("abc"))
(test-re #/(?!^)abc/ "abc" #f)
(test-re #/(?!^^).../ "^^abc" '("^ab"))
(test-re #/(?!^^).../ "^abc" '("abc"))
(test-re #/(?=^)abc/ "abc" '("abc"))
(test-re #/(?=^)abc/ "the abc" #f)
(test-re #/(?=^^).../ "^abc" '("^ab"))
(test-re #/(?=^^).../ "^^abc" '("^^a"))
(test-re #/^(?=$).*/ "" '(""))
(test-re #/^(?!$).*/ "" #f)
...
(test-re #/(?<=(?<!foo)bar)baz/ "foobarbaz" #f)
(test-re #/(?<=\d{3})(?<!999)foo/ "865foo" '("foo"))
(test-re #/(?<=\d{3})(?<!999)foo/ "999foo" #f)
