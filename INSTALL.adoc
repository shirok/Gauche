...
ifdef::JA[]
Gaucheは内部文字エンコーディングとして UTF-8 を使います。

`--enable-multibyte=ENCODING` というオプションをconfigureに渡すことで、
内部文字エンコーディングを変えることができます。ENCODINGには `utf-8` (デフォルト)、
`euc-jp`、`sjis`、そして `none` (8ビット固定長。ISO8859-xなど)を指定できます。
ただし、utf-8以外の内部エンコーディングは非推奨で、既にGaucheをそのエンコーディングで
使っているシステムの互換性のためだけに残されています。Gaucheをutf-8以外の
内部エンコーディングで使っている場合は、移行を考えて下さい。

内部エンコーディングがutf-8であって、コードやデータにそれ以外のエンコーディングを
使うことに制限はありません。入出力の際にエンコーディングを指定しない場合のデフォルトが
utf-8になるというのが主な違いです。詳細は以下のURLで。
endif::JA[]
ifdef::EN[]
Gauche is compiled with UTF-8 as a native character encoding.

You can select different character encoding with `--enable-multibyte=ENCODING`
configure option, where ENCODING can be either `utf-8` (default),
`euc-jp`, `sjis` or `none` (8-bit/character encoding such as ISO8859-x).
However, it is deprecated and kept only for the
backward compatibility to support existing systems.  If your system
uses Gauche with different internal encodings other than utf-8, consider
migration.

Even if you configure Gauche with utf-8, you can still have your code
and data in other encodings.  The main difference is that when you do
I/O without specifying encoding, utf-8 is assumed by default.
See the following URL for the details.
endif::EN[]

https://practical-scheme.net/gauche/gmemo/?Utf8Only
...
ifdef::JA[]
Gaucheのセキュアソケット通信サポートにはいくつかオプションがあります。
ひとつはlink:https://tls.mbed.org/[MbedTLS]
ライブラリ、もうひとつはlink:http://axtls.sourceforge.net/[AxTLS]
ライブラリです。
...
AxTLSは小さなライブラリで、ソースがGaucheにバンドルされているので、
外部ライブラリなしでTLSサポートを組み込めます。ただ、
AxTLSはサポートしている暗号スイートに限りがあり、
相手が正当な証明書を持っていても接続できないことがあります。
どうしてもMbedTLSが使えない場合の最終手段と考えてください。
endif::JA[]
ifdef::EN[]
Gauche includes secure socket connection support.
One usesd link:https://tls.mbed.org/[MbedTLS] library, and the other
uses link:http://axtls.sourceforge.net/[AxTLS] library.
...
AxTLS is a small library and its source is included in the Gauche source
tree.  It can be included in Gauche without relying on external libraries
at runtime.  However, it has limited cipher and hash algorithm support.
We suggest to use it as a fallback if you can't use MbedTLS.
endif::EN[]

[horizontal]
`--with-tls=none`          :: do not include TLS/SSL support
`--with-tls=mbedtls`       :: include only axTLS support
`--with-tls=axtls,mbedtls` :: include TLS/SSL support using
