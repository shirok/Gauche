...
typedef void (*ScmDynLoadEntry)(void);
...
ScmObj internal_ffi_call(
    ...
    int8_t vals1[c_nargs];
    uint8_t vals2[c_nargs];
    int16_t vals3[c_nargs];
    uint16_t vals4[c_nargs];
    int32_t vals5[c_nargs];
    uint32_t vals6[c_nargs];
    int64_t vals7[c_nargs];
    uint64_t vals8[c_nargs];
    char vals9[c_nargs];
    unsigned char vals10[c_nargs];
    short vals11[c_nargs];
    unsigned short vals12[c_nargs];
    int vals13[c_nargs];
    unsigned int vals14[c_nargs];
    long vals15[c_nargs];
    unsigned long vals16[c_nargs];
    float vals17[c_nargs];
    double vals18[c_nargs];
    void *vals20[c_nargs];
    ...
    for(int i = 0; i < c_nargs; i++) {
        ...
        switch(SCM_INT_VALUE(type)) {
            case 1:
                ...
                vals1[i] = (int8_t)SCM_INT_VALUE(value);
                c_avalues[i] = &vals1[i];
            break;
            case 2:
            ...
            case 3:
                ...
                vals3[i] = (int16_t)SCM_INT_VALUE(value);
                c_avalues[i] = &vals3[i];
            break;
            case 4:
                c_atypes[i] = &ffi_type_uint16;
                vals4[i] = (uint16_t)SCM_INT_VALUE(value);
                c_avalues[i] = &vals4[i];
            break;
            case 5:
                c_atypes[i] = &ffi_type_sint32;
                vals5[i] = (int32_t)SCM_INT_VALUE(value);
                c_avalues[i] = &vals5[i];
            break;
            case 6:
                c_atypes[i] = &ffi_type_uint32;
                vals6[i] = (uint32_t)SCM_INT_VALUE(value);
                c_avalues[i] = &vals6[i];
            break;
            case 7:
                c_atypes[i] = &ffi_type_sint64;
                vals7[i] = (int64_t)SCM_INT_VALUE(value);
                c_avalues[i] = &vals7[i];
            break;
            case 8:
            ...
            case 9:
                ...
                vals9[i] = (char)SCM_INT_VALUE(value);
                c_avalues[i] = &vals9[i];
            break;
            case 10:
                c_atypes[i] = &ffi_type_uchar;
                vals10[i] = (unsigned char)SCM_INT_VALUE(value);
                c_avalues[i] = &vals10[i];
            break;
            case 11:
                c_atypes[i] = &ffi_type_sshort;
                vals11[i] = (short)SCM_INT_VALUE(value);
                c_avalues[i] = &vals11[i];
            break;
            case 12:
                c_atypes[i] = &ffi_type_ushort;
                vals12[i] = (short)SCM_INT_VALUE(value);
                c_avalues[i] = &vals12[i];
            break;
            case 13:
            ...
            case 14:
                c_atypes[i] = &ffi_type_sint;
                vals14[i] = (unsigned int)SCM_INT_VALUE(value);
                c_avalues[i] = &vals14[i];
            break;
            case 15:
                c_atypes[i] = &ffi_type_slong;
                vals15[i] = (long)SCM_INT_VALUE(value);
                c_avalues[i] = &vals15[i];
            break;
            case 16:
                c_atypes[i] = &ffi_type_slong;
                vals16[i] = (unsigned long)SCM_INT_VALUE(value);
                c_avalues[i] = &vals16[i];
            break;
            // FIXME
            case 17:
                c_atypes[i] = &ffi_type_slong;
                vals17[i] = (float)SCM_INT_VALUE(value);
                c_avalues[i] = &vals17[i];
            break;
            // FIXME
            case 18:
                c_atypes[i] = &ffi_type_slong;
                vals18[i] = (double)SCM_INT_VALUE(value);
            ...
            case 20:
                ...
                break;
            default:
                Scm_Error("Undefined argument type: %i", SCM_INT_VALUE(type));
            break;
        }
    }
    ...
    switch(SCM_INT_VALUE(rtype)) {
      ...
      default:
        printf("Undefined return type: %li\n", SCM_INT_VALUE(rtype));
        c_rtype = &ffi_type_pointer;
      break;
    }

    ffi_prep_cif(&cif, FFI_DEFAULT_ABI, c_nargs, c_rtype, c_atypes);

    void *c_fn = SCM_FOREIGN_POINTER_REF(void*, fn);
    void *rvalue = malloc(SCM_INT_VALUE(rvalue_size));
    ffi_call(&cif, FFI_FN(c_fn), rvalue, c_avalues);
    return Scm_MakeForeignPointer(pointer_class, rvalue);
}

ScmObj internal_address(ScmObj pointer, int offset) {
