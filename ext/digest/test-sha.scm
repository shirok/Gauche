...
(define (test-from-file file)
  (match-let1 (input . results) (parse-infofile file)
    (if-let1 r (ref results 'sha1 #f)
      (test* #"~file sha1" r (digest-string-to 'hex <sha1> input)))
    (if-let1 r (ref results 'sha224 #f)
      (test* #"~file sha224" r (digest-string-to 'hex <sha224> input)))
    (if-let1 r (ref results 'sha256 #f)
      (test* #"~file sha256" r (digest-string-to 'hex <sha256> input)))
    (if-let1 r (ref results 'sha384 #f)
      (test* #"~file sha384" r (digest-string-to 'hex <sha384> input)))
    (if-let1 r (ref results 'sha512 #f)
      (test* #"~file sha512" r (digest-string-to 'hex <sha512> input)))
    ))
