...
(test* "<deflate-port>" <deflate-port>
       (class-of (open-deflate-port (open-output-string))))
(test* "<deflate-port>'s CPL" '(<deflate-port> <port> <top>)
       (map class-name 
            (class-precedence-list
             (class-of (open-deflate-port (open-output-string))))))

(test* "open-deflate-port" #t
       (port? (open-deflate-port (open-output-string))))

(test* "open-deflate-port" *test-error*
       (open-deflate-port (standard-input-port)))

(test* "open-deflate-port :compression-level 9" #t
       (port? (open-deflate-port (open-output-string) :compression-level 9)))
(test* "open-deflate-port :compression-level 10" 'OK
       (guard (e ((<zlib-stream-error> e) 'OK)
                 (else 'error))
         (open-deflate-port (open-output-string) :compression-level 10)
         'error))

(test* "open-deflate-port :buffer-size 512" #t
       (port? (open-deflate-port (open-output-string) :buffer-size 512)))

(test* "open-deflate-port :window-bits 15" #t
       (port? (open-deflate-port (open-output-string) :window-bits 15)))
(test* "open-deflate-port :window-bits -1" 'OK
       (guard (e ((<zlib-stream-error> e) 'OK))
         (open-deflate-port (open-output-string) :window-bits -1)))

(test* "open-deflate-port :memory-level 1" #t
       (port? (open-deflate-port (open-output-string) :memory-level 1)))
(test* "open-deflate-port :memory-level -1" 'OK
       (guard (e ((<zlib-stream-error> e) 'OK))
         (open-deflate-port (open-output-string) :memory-level -1)))

(test* "open-deflate-port :strategy Z_HUFFMAN_ONLY" #t
       (port? (open-deflate-port (open-output-string) :strategy Z_HUFFMAN_ONLY)))
(test* "open-deflate-port :strategy 100" 'OK
       (guard (e ((<zlib-stream-error> e) 'OK))
         (open-deflate-port (open-output-string) :strategy 100)))

(test* "open-deflate-port :dictionary \"abc\"" #t
       (port? (open-deflate-port (open-output-string) :dictionary "abc")))
(test* "open-deflate-port :dictionary '()" *test-error*
       (open-deflate-port (open-output-string) :dictionary '()))

(test* "zstream-dictionary-adler32" #f
       (zstream-dictionary-adler32
        (open-deflate-port (open-output-string))))
(test* "zstream-dictionary-adler32" (adler32 "abc")
       (zstream-dictionary-adler32
        (open-deflate-port (open-output-string) :dictionary "abc")))
...
(test* "owner? keyword" #f
       (let1 p (open-output-string)
         (close-output-port (open-deflate-port p)) 
         (port-closed? p)))

(test* "owner? keyword" #t
       (let1 p (open-output-string)
         (close-output-port (open-deflate-port p :owner? #t))
         (port-closed? p)))

(test* "port-file-number" #f
       (port-file-number (open-deflate-port (open-output-string))))

(test* "port-file-number" 'OK
       (call-with-output-file "test.o"
         (lambda (in)
           (let1 dp (open-deflate-port in)
             (begin0 (and (number? (port-file-number dp)) 'OK)
                     (close-output-port dp))))))
(sys-unlink "test.o")

(test* "port-name" "[deflate (output string port)]"
       (port-name (open-deflate-port (open-output-string))))
...
(test* "deflate-port-full-flush" #*"x\x9cJLJ\x06\0\0\0\xff\xffKIM\x03\0\x08\x1e\x02V"
       (call-with-output-string
         (lambda (p1)
           (let1 p2 (open-deflate-port p1)
             (display "abc" p2)
             (deflate-port-full-flush p2)
             (display "def" p2)
...
(test* "<inflate-port>" <inflate-port>
       (class-of (open-inflate-port (open-input-string ""))))
(test* "<inflate-port>'s CPL" '(<inflate-port> <port> <top>)
       (map class-name 
            (class-precedence-list
             (class-of (open-inflate-port (open-input-string ""))))))

(test* "open-inflate-port" #t
       (port? (open-inflate-port (open-input-string ""))))

(test* "open-inflate-port" *test-error*
       (open-inflate-port (open-output-string)))

(test* "open-inflate-port" #t
       (port? (open-inflate-port (open-input-string "") :buffer-size 512)))

(test* "open-inflate-port :window-bits 8" #t
       (port? (open-inflate-port (open-input-string "") :window-bits 8)))
(test* "open-inflate-port :window-bits -1" 'OK
       (guard (e ((<zlib-stream-error> e) 'OK))
         (open-inflate-port (open-input-string "") :window-bits -1)))

(test* "open-inflate-port :dictionary \"abc\"" "abc"
       (let1 s (call-with-output-string
                 (lambda (p)
                   (let1 p2 (open-deflate-port p :dictionary "abc")
                     (display "abc" p2)
                     (close-output-port p2))))
         (port->string
          (open-inflate-port (open-input-string s) :dictionary "abc"))))
...
(test* "EOF" (read-from-string "")
       (let1 p (open-inflate-port
                (open-input-string (deflate-string "foobar")))
         (port->string p)
         (read-char p)))

(test* "zstream-total-in" 14
       (let1 p (open-inflate-port (open-input-string #*"x\x9cK\xcb\xcfOJ,\x02\0\x08\xab\x02z"))
         (port->string p)
         (zstream-total-in p)))
...
(test* "read gzip'ed stream" "foobar"
       (let ((str #*"\x1f\x8b\x08\0:c\xc3C\x02\x03K\xcb\xcfOJ,\x02\0\x95\x1f\xf6\x9e\x06\0\0\0"))
         (port->string (open-inflate-port (open-input-string str)
                                          :window-bits (+ 15 16)))))
...
(test* "owner? keyword" #t
       (let1 p (open-input-string (deflate-string "foo"))
         (close-input-port (open-inflate-port p :owner? #t))
         (port-closed? p)))

(test* "owner? keyword" #f
       (let1 p (open-input-string (deflate-string "foo"))
         (close-input-port (open-inflate-port p))
         (port-closed? p)))
...
(test* "port-name" "[inflate (input string port)]"
       (port-name (open-inflate-port (open-input-string ""))))

(test* "port-file-number" #f
       (port-file-number (open-inflate-port (open-input-string ""))))

(call-with-output-file "test.o"
  (lambda (o)
    (close-output-port (open-deflate-port o))))

(test* "port-file-number" 'OK
       (call-with-input-file "test.o"
         (lambda (in)
           (let1 p (open-inflate-port in)
...
(test* "broken data" 'OK
       (guard (e ((and (<zlib-data-error> e)
                       (<io-read-error> e))
                  'OK))
         (port->string (open-inflate-port (open-input-string "abc")))
         'error))
