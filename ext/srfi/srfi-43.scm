...
(define-inline (nonneg-int? x)
  (and (integer? x)
       (not (negative? x))))

(define-inline (between? x y z)
  (and (<  x y)
       (<= y z)))

(define (vectors-ref vectors i)
  (map (^v (vector-ref v i)) vectors))
...
(define (check-index vec index callee)
  (let1 index (check-type integer? index callee)
    (cond [(< index 0)
           (check-index vec
...
(define %smallest-length
  (rec (loop vector-list length callee)
    (if (null? vector-list)
      length
...
(define %vector-reverse-copy!
  (letrec ([loop (^[target source sstart i j]
                   (when (<= sstart i)
                     (vector-set! target j (vector-ref source i))
...
(define %vector-fold1
  (letrec ([loop (^[kons knil len vec i]
                   (if (= i len)
                     knil
                     (loop kons
...
;;;     (KONS <index> <knil> <elt> ...) -> knil'
(define %vector-fold2+
  (letrec ([loop (^[kons knil len vectors i]
                   (if (= i len)
                     knil
                     (loop kons
                           (apply kons i knil
...
;  (if (null? args)
;      (values 0 (vector-length vec) (undefined))
;      (let ((start (check-index vec (car args) vector-copy)))
;        (if (null? (cdr args))
;            (values start (vector-length vec) (undefined))
...
(define (vector-reverse-copy vec . maybe-start+end)
  (let-vector-start+end vector-reverse-copy vec maybe-start+end
                        (start end)
    (rlet1 new (make-vector (- end start))
...
(define (vector-empty? vec)
  (let1 vec (check-type vector? vec vector-empty?)
...
(define (vector= elt=? . vectors)
  (let1 elt=? (check-type procedure? elt=? vector=)
    (cond [(null? vectors)
           #t]
          [(null? (cdr vectors))
...
(define vector-fold-right
  (letrec ([loop1 (^[kons knil vec i]
                    (if (negative? i)
                      knil
                      (loop1 kons (kons i knil (vector-ref vec i))
                             vec
                             (- i 1))))]
           [loop2+ (^[kons knil vectors i]
                     (if (negative? i)
                       knil
                       (loop2+ kons
                               (apply kons i knil
...
(define (vector-skip-right pred? vec . vectors)
  (vector-index/skip-right (^ elts (not (apply pred? elts)))
                           vec vectors
                           vector-index-right))

(define vector-index/skip-right
  (letrec ([loop1  (^[pred? vec i]
                     (cond [(negative? i) #f]
                           [(pred? (vector-ref vec i)) i]
...
(define vector-any
  (letrec ([loop1 (^[pred? vec i len len-1]
                    (and (not (= i len))
                         (if (= i len-1)
                           (pred? (vector-ref vec i))
                           (or (pred? (vector-ref vec i))
                               (loop1 pred? vec (+ i 1) len len-1)))))]
           [loop2+ (^[pred? vectors i len len-1]
                     (and (not (= i len))
                          (if (= i len-1)
                            (apply pred? (vectors-ref vectors i))
                            (or (apply pred? (vectors-ref vectors i))
                                (loop2+ pred? vectors (+ i 1) len len-1)))))])
    (^[pred? vec . vectors]
...
(define (vector-swap! vec i j)
  (let1 vec (check-type vector? vec vector-swap!)
...
;;;
;;; [SK] Gauche supports the extended version natively.
;(define list->vector
