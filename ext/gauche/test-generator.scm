...
(let ()
  (define (fill-take lis left right)
    (if (> left (length lis))
                    ...
                    [(t dir fn data)
                     (let ([expect (integer->list data)]
                       ...
                       (begin (test* (format "~a ~a" 'fn data) expect
                              ...
                              (test* (format "~a (8,_) ~a" 'fn data)
                                     (cv (fill-take expect 8 0))
                                     (generator->list (fn data 8)))
                              (test* (format "~a (16,8) ~a" 'fn data)
                                     (cv (fill-take expect 16 8))
                                     (generator->list (fn data 16 8)))))])))
    (t f bits->generator #x36)
    (t r reverse-bits->generator #x36)
    (t f bits->generator #xcafebabe)
    (t r reverse-bits->generator #xcafebabe)
    ))
