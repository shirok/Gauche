...
(define (array-concatenate a b . opt)
  (let* ((dimension (get-optional opt 0))
         (a-start (start-vector-of a))
...
(define (array-flip! a . opt)
  (let* ((dimension (get-optional opt 0))
         (lo (s32vector-ref (start-vector-of a) dimension))
         (end (end-vector-of a))
...
(define (pretty-print-array a . opt)
  (let* ((p1 (get-optional opt (current-output-port)))
         (p (cond ((port? p1) p1)
    ...
    (let-keywords* (if (pair? opt) (cdr opt) '())
      ...
      (let ((fixed-width (+ (if left (print-width left) 0)
        ...
        (define (column-widths vec)
          (let loop1 ((col (caddr inner-shape))
                      (res '()))
            (cond
              ((< col ncols)
               (vector-set! vec col-dim col)
               (let loop2 ((row (car inner-shape))
                           (x 0))
                 (cond
                   ((< row nrows)
          ...
          (let* ((widths (if fill? (column-widths (vector-copy vec)) '()))
            ...
            (let loop1 ((row (car inner-shape)))
              (when (< row nrows)
                ...
                (vector-set! vec row-dim row)
                (let loop2 ((col (caddr inner-shape))
                            (w widths))
                  (when (< col ncols)
                    (vector-set! vec col-dim col)
                    (display (if (pair? w)
...
(define (make-minimal-backend-array ls sh . args)
      ...
      (else
       (let ((a-s? (signed? a))
         ...
         (if (or (zero? a-size) (zero? b-size))
           <array>
           (let ((res-size (max a-size b-size)))
             (backend (or a-s? b-s?)
  ...
  (let loop ((class (class-of (car ls))) (l ls))
    (if (null? l)
      (return class)
      (let ((next (join class (class-of (car l)))))
        (if (eq? next <array>)
