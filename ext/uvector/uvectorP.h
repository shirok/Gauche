 ...
 *  $Id: uvectorP.h,v 1.5 2002-04-15 22:04:59 shirok Exp $
 */
...
static SCM_UVECTOR_INT32 scm_s32vector_unbox(ScmObj obj)
{
    ...
    if (SCM_NULLP(s32_min) || SCM_NULLP(s32_max)) {
        ScmObj two_31 = Scm_Ash(SCM_MAKE_INT(1), 31);
        s32_min = Scm_Negate(two_31);
        s32_max = Scm_Subtract2(two_31, SCM_MAKE_INT(1));
    }

    if (SCM_INTP(obj)) {
        return SCM_INT_VALUE(obj);
    } else if(SCM_BIGNUMP(obj) && SCM_NUM_BETWEEN(s32_min, obj, s32_max)) {
...
static SCM_UVECTOR_UINT32 scm_u32vector_unbox(ScmObj obj)
{
    ...
    if (SCM_NULLP(u32_min) || SCM_NULLP(u32_max)) {
        ScmObj two_32 = Scm_Ash(SCM_MAKE_INT(1), 32);
        u32_min = SCM_MAKE_INT(0);
        u32_max = Scm_Subtract2(two_32, SCM_MAKE_INT(1));
    }

    if (SCM_INTP(obj) && SCM_INT_VALUE(obj) >= 0) {
        return SCM_INT_VALUE(obj);
    } else if(SCM_BIGNUMP(obj) && SCM_NUM_BETWEEN(u32_min, obj, u32_max)) {
...
{
    static ScmObj s64_min = SCM_NIL;
    static ScmObj s64_max = SCM_NIL;

    if (SCM_NULLP(s64_min) || SCM_NULLP(s64_max)) {
        ScmObj two_63 = Scm_Ash(SCM_MAKE_INT(1), 63);
        s64_min = Scm_Negate(two_63);
        s64_max = Scm_Subtract2(two_63, SCM_MAKE_INT(1));
    }

    if (SCM_INTP(obj)) {
        return SCM_INT_VALUE(obj);
    } else if (SCM_BIGNUMP(obj) && SCM_NUM_BETWEEN(s64_min, obj, s64_max)) {
...
{
    static ScmObj u64_min = SCM_NIL;
    static ScmObj u64_max = SCM_NIL;

    if (SCM_NULLP(u64_min) || SCM_NULLP(u64_max)) {
        ScmObj two_64 = Scm_Ash(SCM_MAKE_INT(1), 64);
        u64_min = SCM_MAKE_INT(0);
        u64_max = Scm_Subtract2(two_64, SCM_MAKE_INT(1));
    }

    if (SCM_INTP(obj) && SCM_INT_VALUE(obj) >= 0) {
        return SCM_INT_VALUE(obj);
    } else if (SCM_BIGNUMP(obj) && SCM_NUM_BETWEEN(u64_min, obj, u64_max)) {
