#
# Generate uvutil.c
# $Id: uvutil.c.sh,v 1.9 2002-06-25 08:32:21 shirok Exp $
#
                              ...
                              ScmObj max)
{
    ...
    } else if (SCM_PAIRP(min)) {
        smin = min;
        if (Scm_Length(smin) != len) {
            Scm_Error("List length doesn't match the target vector: %S and %S", min, v0);
        }
    } else if (${OBJCHKP}(min)) {
    ...
    } else if (SCM_PAIRP(max)) {
        smax = max;
        if (Scm_Length(smax) != len) {
            Scm_Error("List length doesn't match the target vector: %S and %S", max, v0);
        }
    } else if (${OBJCHKP}(max)) {
