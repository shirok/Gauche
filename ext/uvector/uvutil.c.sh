#
# Generate uvutil.c
# $Id: uvutil.c.sh,v 1.4 2002-06-20 06:53:34 shirok Exp $
#
...
#define CLAMP_LO_RET(val, clamp) \
    do { if (CLAMP_LO_P(clamp)) return (val); else OVERFLOW; } while (0)
#define CLAMP_HI_RET(val, clamp) \
    do { if (CLAMP_HI_P(clamp)) return (val); else OVERFLOW; } while (0)
...
{
    long r = x + y;
    if (y >= 0) { if (x >= 0 && r < 0) CLAMP_HI_RET(LONG_MAX, clamp); }
    else        { if (x < 0 && r >= 0) CLAMP_LO_RET(LONG_MIN, clamp); }
    return r;
}
...
static u_long uadd(u_long x, u_long y, int clamp)
{
    u_long r, c = 0;
    UADD(r, c, x, y);
    if (c) CLAMP_HI_RET(SCM_ULONG_MAX, clamp);
    return r;
}
...
static long ssub(long x, long y, int clamp)
{
    long r = x - y;
    if (x >= 0) {
        if (y < 0 && r < 0)   CLAMP_HI_RET(LONG_MAX, clamp);
    } else {
        if (y >= 0 && r >= 0) CLAMP_LO_RET(LONG_MIN, clamp);
    }
    return r;
}
...
static u_long usub(u_long x, u_long y, int clamp)
{
    u_long r, c = 0;
    USUB(r, c, x, y);
    if (c) CLAMP_LO_RET(0, clamp);
    return r;
}

/* u_long multiplication.  NB: clampp is a boolean, not the bitflag.
   see smul below.  */
static u_long umul(u_long x, u_long y, int clampp)
{
    u_long r;
    if (x==0 || y==0) return 0;
    r = x*y;
    if (r<x || r<y) {
        if (clampp) return SCM_ULONG_MAX;
        else OVERFLOW;
    }
    return r;
}
...
static long smul(long x, long y, int clamp)
{
    ...
    if (x >= 0) {
        if (y >= 0) {
            kr = umul(x, y, CLAMP_HI_P(clamp));
            if (kr > LONG_MAX) CLAMP_HI_RET(LONG_MAX, clamp);
            else return kr;
        } else {
            kr = umul(x, -y, CLAMP_LO_P(clamp));
            if (kr > LONG_MAX+1UL) CLAMP_LO_RET(LONG_MIN, clamp);
            else return -kr;
        }
    } else {
        if (y >= 0) {
