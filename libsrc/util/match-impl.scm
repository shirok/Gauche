...
(define-syntax match-let
  (er-macro-transformer
   (^[f r c]
     (define args (cdr f))
     (define g158 (lambda (pat exp body)
                    (quasirename r
                      `(match ,exp (,pat ,@body)))))
     (define g154 (lambda (pat exp body)
     ...
     (define g146 (lambda ()
                    (error "syntax error in" `(match-let ,@args))))
     (define g145 (lambda (p1 e1 p2 e2 body)
                    (let ((g1 (gensym)) (g2 (gensym)))
                      (quasirename r
                        `(let ((,g1 ,e1) (,g2 ,e2))
                           (match (cons ,g1 ,g2) ((,p1 . ,p2) ,@body)))))))
     (define g136 (cadddr match:expanders))
     (if (pair? args)
...
(define-syntax match-let1 ;; Gauche extension
  (er-macro-transformer
   (^[f r c]
     (let ([pat (cadr f)]
           [exp (caddr f)]
...
(define-syntax match-letrec
  (er-macro-transformer
   (^[f r c]
     (define args (cdr f))
     (define g200 (cadddr match:expanders))
     (define g199 (lambda (p1 e1 p2 e2 body)
                    (quasirename r
                      `(match-letrec (((,p1 . ,p2) (cons ,e1 ,e2))) ,@body))))
     (define g195 (lambda ()
                    (error "syntax error in" `(match-letrec ,@args))))
     (define g194 (lambda (pat exp body)
                    (quasirename r
                      `(match-letrec
                           ((,(list->vector pat) (vector ,@exp)))
                         ,@body))))
     (define g186 (lambda (pat exp body)
     ...
     (if (pair? args)
       (if (list? (car args))
         (if (match:every
              ...
              (car args))
           (if (and (list? (cdr args)) (pair? (cdr args)))
             ((lambda () (quasirename r `(letrec ,@args))))
...
(define-syntax match-define
  (er-macro-transformer
   (^[f r c]
     (define args (cdr f))
     (define g210 (cadddr match:expanders))
     (define g209 (lambda ()
                    (error "syntax error in" `(match-define ,@args))))
     (if (pair? args)
       (if (g210 (car args))
         (if (and (pair? (cdr args)) (null? (cddr args)))
           ((lambda () (quasirename r `(begin (define ,@args)))))
           (g209))
         (if (and (pair? (cdr args)) (null? (cddr args)))
           ((lambda (pat exp)
