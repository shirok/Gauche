...
;; Break properties.  The actual property values are in
;; src/gauche/priv/unicode_attr.h, which is generated by src/gen-unicode.scm.
;; NB: We use 'Other instead of 'Any in UAX#29.
;; NB: 'ExtPict is not defined in Grapheme Break Properties, but in
;; Emoji Properties.  It is required to implement GB11 and WB3c  rule
...
(define (ucd-word-break-properties)
  '(CR LF Newline Extend ZWJ Regional_Indicator
    Format Katakana Hebrew_Letter ALetter Single_Quote Double_Quote
    MidLetter MidNum MidNumLet Numeric ExtendNumLet WSegSpace
    ExtPict ExtPict_ALetter Other))
...
(define (augument-break-property file db)
  (define (ensure c)
    (let1 e (ensure-ucd-break-property db c)
      (case (ucd-break-property-grapheme e)
        [(Other) (set! (ucd-break-property-grapheme e) 'ExtPict)]
        [else (warn "Overriding non-'Other' grapheme property of U+~4,'0x (~s)\n"
                    c (ucd-break-property-grapheme e))])
      (case (ucd-break-property-word e)
        [(Other)   (set! (ucd-break-property-word e) 'ExtPict)]
        [(ALetter) (set! (ucd-break-property-word e) 'ExtPict_ALetter)]
        [else (warn "Overriding non-'Other' word property of U+~4,'0x (~s)\n"
                    c (ucd-break-property-word e))])))
  (define (handle line)
    (rxmatch-case line
